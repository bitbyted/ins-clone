...
import {FormContainer, FormButton, FormInput, FormLabel, ErrMessage} from './Profile.styles';
...
export default function CreateProfile({userID, setIsProfileCreated}) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.post('/api/profiles', formDataToSubmit, {headers: {'Content-Type': 'multipart/form-data'}});
    ...
    <FormContainer>
      ...
      <form onSubmit={handleSubmit}>
        <div>
          ...
          <FormInput type='file' id='profilePic' name='profilePic' onChange={handleImageChange} />
        </div>
        <div>
          <FormLabel htmlFor='name'>Name</FormLabel>
          <FormInput type='text' id='name' name='name' value={formData.name || ''} onChange={handleChange} />
        </div>
        <div>
          <FormLabel htmlFor='category'>Category</FormLabel>
          <FormInput type='text' id='category' name='category' value={formData.category || ''} onChange={handleChange} />
        </div>
        <div>
          <FormLabel htmlFor='bio'>Bio</FormLabel>
          <FormInput type='text' id='bio' name='bio' value={formData.bio || ''} onChange={handleChange} />
        </div>
        <div>
