...
export const postDataSlice = createSlice({
  ...
  reducers: {
    ...
    handleLikeSingleClick: (state, action) => {
      ...
      const postIndex = state.postData.findIndex((post) => post.postID === updatedPost.postID);
    ...
    handleLikeDoubleClick: (state, action) => {
      ...
      const postIndex = state.postData.findIndex((post) => post.postID === postID);
      if (postIndex !== -1) {
        state.postData[postIndex] = updatedPost;
      }
      setTimeout(() => {
...
export const {savePostData, handleLikeSingleClick, handleLikeDoubleClick, postComment} = postDataSlice.actions;
